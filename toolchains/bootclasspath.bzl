...
def _bootclasspath_impl(ctx):
    ...
    # Use Java's source-file launcher (JDK 11+) to compile and run in a single step.
    # This avoids creating an intermediate tree artifact (declared directory) which seems
    # to be causing some trouble sometimes.
